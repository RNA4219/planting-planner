...
3. UI 実装: 市場切替コンポーネントとカテゴリタブのステート連携。
   ...
   - [ ] RecommendationsTable 仮想化リプレース: `frontend/tests/components/RecommendationsTable.virtualization.test.tsx` に `react-window` の `FixedSizeList` を前提としたレンダリング数検証・キーボード操作の保持テストを追加して赤くし、`frontend/src/components/RecommendationsTable.tsx` を仮想スクロール実装へ置換して緑化する。置換後は `npm run test -- RecommendationsTable.virtualization` を通した上で、`npm run build` による型チェック・バンドル検証を行う。
     - パフォーマンス検証: `frontend/scripts/benchmarks/recommendationsTable.web-vitals.ts` で Web Vitals (`LCP` / `FID` / `CLS`) を `react-window` 版と現行版で計測し、`docs/reports/perf/recommendationsTable.csv` に結果を追記する。`npm run bench:web-vitals -- --report=docs/reports/perf/recommendationsTable.csv` 実行時に LCP/FID が ±5% 以内に収まり、CLS が悪化しないことを受け入れ条件とする。
     - React Profiler: `frontend/scripts/profiler/recommendationsTable.profile.ts` を追加し、`npm run profile -- recommendationsTable` で commit 時間・レンダー回数を取得する。プロファイル比較でコミット時間の中央値が 20% 以上改善するか、レンダー回数が 30% 以上削減されていることを完了条件に含める。
   - [ ] RecommendationsTable 用 React Query キャッシュキー最適化: `frontend/tests/hooks/recommendations/useRecommendationsQuery.cacheKey.test.ts` に市場 ID・カテゴリ・ソート条件ごとのキャッシュキーがユニークかつ再フェッチ条件が網羅されることを確認するテストを追加して赤くする。`frontend/src/hooks/recommendations/useRecommendationsQuery.ts` の `useQuery` 呼び出しを新キーへ更新し、`npm run test -- useRecommendationsQuery.cacheKey` を緑化する。
     - ベンチマーク: キャッシュヒット率は `frontend/scripts/benchmarks/react-query-cache.ts` で `npm run bench:react-query-cache` を実行し、Web Vitals の `FID` が ±5% 以内であることと、ヒット率が 90% 以上であることを受け入れ基準とする。React Profiler での比較でもデータフェッチによる再レンダーが 50% 以上削減されていることを確認する。
   - [ ] RecommendationsTable メモ化戦略 (useMemo) 検討: `frontend/tests/components/RecommendationsTable.memoization.test.tsx` を作成し、入力データが変わらない場合にレンダー回数が増加しないこと、ソート条件変更時に派生データが再計算されることを先にテスト化して赤くする。`frontend/src/components/RecommendationsTable.tsx` へ `useMemo` / `useCallback` を追加し、テストを緑化する。
     - 性能評価: `npm run bench:web-vitals` と `npm run profile -- recommendationsTable` を再実行して LCP/FID の悪化が ±5% 以内、React Profiler の commit 時間中央値が 10% 以上改善していることを確認する。測定ログは `docs/reports/perf/recommendationsTable-memoization.csv` / `.json` に追記し、タスク完了の受け入れ条件とする。
...
4. QA: React Testing Library 結合テスト更新と Playwright シナリオ作成。
   ...
   - [x] Playwright フレーク監視ダッシュボード整備: [週4計画](./ROADMAP.md) に沿って CI 成果のメトリクス収集を自動化し、`frontend e2e (playwright)` ジョブで最新成功率を可視化する。
   ...
   - 完了理由: `docs/spec/v0.3/ARCHITECTURE.md` の品質項目で `frontend e2e (playwright)` 表記と `--job-name` 指定必須を反映済み。
