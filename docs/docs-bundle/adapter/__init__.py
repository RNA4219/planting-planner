...
class AggregationStrategy(Generic[T]):
    ...
    def __call__(self, responses: Sequence[T]) -> T:
...
class MajorityVoteStrategy(AggregationStrategy[T]):
    def __init__(
        ...
        *,
        ...
        tie_breaker: Callable[[Sequence[T]], T] | None = None,
    ) -> None:
    ...
    def aggregate(self, responses: Sequence[T]) -> T:
        if not responses:
            raise AggregationError("cannot aggregate an empty sequence")
        ...
        for item in responses:
            if self._key(item) in winners:
                return item
        if self._tie_breaker is not None:
            return self._tie_breaker(responses)
...
class ProviderRegistry:
    ...
    def register(self, spec: ProviderSpec, *, override: bool = False) -> None:
        if spec.name in self._providers and not override:
            raise ProviderRegistrationError(f"provider '{spec.name}' is already registered")
    ...
    def as_mapping(self) -> Mapping[str, ProviderSpec]:
