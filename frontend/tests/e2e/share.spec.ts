  ...
  page.addInitScript((flag) => {
    ...
    if (flag) {
      ...
      Object.defineProperty(navigator, 'share', {
        configurable: true,
        value: (data: ShareData) => (calls.push(data), Promise.resolve()),
      })
    } else {
      ...
      Object.defineProperty(navigator, 'clipboard', {
        configurable: true,
        value: { writeText: (text: string) => (clipboardCalls.push(text), Promise.resolve()) },
      })
    }
  }, supported)
...
const setupApi = async (page: Page, telemetry: Telemetry[], recommendRequests?: string[]) => {
  ...
  await context.route('**/sw.js', (route) =>
    route.fulfill({
      ...
      body: 'self.addEventListener("install",()=>self.skipWaiting());self.addEventListener("activate",()=>self.clients.claim());',
    }),
  )
  ...
  await context.route('**/api/recommend?**', async (route) => {
    ...
    await fulfillJson(
      route,
      recommendMap[key] ?? { week: '2024-W30', region: 'temperate', items: [] },
    )
  })
  ...
  const resolveStatus = (route: Route) => {
    ...
    return statusCalls === 1
      ...
      : fulfillJson(route, {
          state: 'success',
          finished_at: '2024-03-10T12:34:00Z',
          updated_records: 3,
          last_error: null,
        })
  }
...
test.describe('共有フロー', () => {
  test('Web Share API 経路', async ({ page }) => {
    ...
    await setupNavigator(page, true)
    await setupApi(page, telemetry, recommendRequests)
    await page.goto('/')
    ...
    const warmRecommendRequest = page.waitForRequest(
      (request) =>
        request.url().includes('/api/recommend') && request.url().includes('region=warm'),
    )
    await page.getByRole('button', { name: 'この条件で見る' }).click()
    await warmRecommendRequest
    expect(
      recommendRequests.some(
        (entry) => entry.includes('region=warm') && entry.includes('category=leaf'),
      ),
    ).toBe(true)
    ...
    await expect
      .poll(() =>
        page.evaluate(
          () =>
            (window as typeof window & { __refreshCalls?: string[] }).__refreshCalls?.length ?? 0,
        ),
      )
      .toBe(1)
    ...
    await expect(toastStack.getByText('更新を開始しました', { exact: false })).toBeVisible({
      timeout: 10000,
    })
    await expect(toastStack.getByText('データ更新が完了しました', { exact: false })).toBeVisible({
      timeout: 10000,
    })
    await page.evaluate(() =>
      (window as typeof window & { __setOnline: (online: boolean) => void }).__setOnline(false),
    )
    const offlineBanner = page.getByTestId('offline-status-banner')
    await expect(offlineBanner).toBeVisible()
    await expect
      .poll(() => telemetry.some((entry) => entry.event === 'offline.banner_shown'))
      .toBe(true)
    const offlineEvent = telemetry.find((entry) => entry.event === 'offline.banner_shown')
    expect.soft(offlineEvent).toBeDefined()
    await page.evaluate(() =>
      (window as typeof window & { __setOnline: (online: boolean) => void }).__setOnline(true),
    )
    await expect(offlineBanner).toBeHidden()
    await page.getByRole('button', { name: '共有' }).click()
    await expect(page.getByText('共有リンクを送信しました')).toBeVisible()
    const shareCalls = await page.evaluate<ShareData[]>(
      () => (window as typeof window & { __shareCalls: ShareData[] }).__shareCalls,
    )
    expect(shareCalls).toHaveLength(1)
    expect(shareCalls[0]).toMatchObject({
      title: 'Planting Planner',
      text: expect.stringContaining('地域: 暖地'),
      url: expect.stringContaining('region=warm'),
    })
  })

  test('Web Share API 非対応時はクリップボードへフォールバックする', async ({ page }) => {
    const telemetry: Telemetry[] = []
    await setupNavigator(page, false)
    await setupApi(page, telemetry)
    await page.goto('/')
    const shareButton = page.getByRole('button', { name: '共有' })
    await expect(shareButton).toBeEnabled()
    await shareButton.click()
    await expect(page.getByText('共有リンクをコピーしました')).toBeVisible()
    const clipboardCalls = await page.evaluate<string[]>(
      () => (window as typeof window & { __clipboardCalls: string[] }).__clipboardCalls,
    )
