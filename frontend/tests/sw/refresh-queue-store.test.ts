...
const recordEnqueue = vi
  .fn<Parameters<Store['recordEnqueue']>[0], ReturnType<Store['recordEnqueue']>>()
  .mockResolvedValue(undefined)
const recordAttempt = vi
  .fn<Parameters<Store['recordAttempt']>[0], ReturnType<Store['recordAttempt']>>()
  .mockResolvedValue(undefined)
const recordSuccess = vi
  .fn<Parameters<Store['recordSuccess']>[0], ReturnType<Store['recordSuccess']>>()
  .mockResolvedValue(undefined)
const recordFailure = vi
  .fn<Parameters<Store['recordFailure']>[0], ReturnType<Store['recordFailure']>>()
  .mockResolvedValue(undefined)
...
vi.mock('workbox-background-sync', () => {
  ...
  class MockBackgroundSyncPlugin {
    ...
    constructor(
      _name: string,
      _options?: { onSync?: ({ queue }: { queue: unknown }) => Promise<void> | void },
    ) {
  ...
  it('records queue lifecycle events on success', async () => {
    ...
    queue.shiftRequest
      .mockResolvedValueOnce({
        request,
        timestamp: pushedEntry?.timestamp,
