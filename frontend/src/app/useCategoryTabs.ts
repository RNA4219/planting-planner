...
export type CategoryTabsResolver = (scope: MarketScope) => readonly CategoryTabDefinition[]

export type CategoryValidator = (scope: MarketScope, category: CropCategory) => CropCategory

export type CategoryTabsUpdater = (markets: readonly MarketScopeOption[]) => void
...
const buildCategoryTabsFromMetadata = (
  ...
  const mapped = categories
    ...
    .filter((entry): entry is CropCategoryEntry => isCropCategory(entry.category.category))
...
const createInitialCategoryTabsMap = (): CategoryTabsMap => {
  ...
  MARKET_SCOPE_FALLBACK_DEFINITIONS.forEach((definition) => {
    const categories =
      definition.categories && definition.categories.length > 0 ? definition.categories : undefined
...
export const useCategoryTabs = (): UseCategoryTabsResult => {
  ...
  const ensureValidCategory: CategoryValidator = useCallback(
    (scope, currentCategory) => {
      ...
      return tabs.some((tab) => tab.key === currentCategory) ? currentCategory : tabs[0]!.key
    },
