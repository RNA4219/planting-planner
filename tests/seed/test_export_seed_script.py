...
try:
    ...
    pytest.skip(reason=f"scripts.export_seed unavailable: {exc}", allow_module_level=True)
...
def sample_payload() -> SeedPayload:
    return SeedPayload(
        crops=[{"id": 1, "name": "Test Crop", "category": "leaf"}],
        price_samples=[{
            "crop_id": 1,
            "week": "2024-W01",
            "avg_price": 120,
...
def test_export_seed_script_writes_metadata(
    ...
    def fake_load_seed_payload(*, data_dir: Path | None = None) -> SeedPayload:  # noqa: ARG001
        calls["load"] += 1
